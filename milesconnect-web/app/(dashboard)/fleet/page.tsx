...
function toFilter(s: ApiVehicle["status"]): VehicleFilter {
  if (s === "ACTIVE") return "Active";
  if (s === "MAINTENANCE") return "Maintenance";
  return "Idle";
}

async function fetchVehicleStatuses(): Promise<VehicleFilter[]> {
  const res = await api.get<{ data: ApiVehicle[] }>("/api/vehicles");
  const vehicles = res?.data ?? [];
  return vehicles.map((v) => toFilter(v.status));
}
...
export default function FleetMapPage() {
  ...
  const vehiclesQuery = useQuery({
    ...
    queryFn: fetchVehicleStatuses,
  });

  const vehicleStatuses = vehiclesQuery.data ?? [];
  const counts = useMemo(() => {
    ...
    for (const s of vehicleStatuses) out[s] += 1;
    return out;
  }, [vehicleStatuses]);
  ...
  const filteredTotal = useMemo(() => {
    ...
    let n = 0;
    for (const s of vehicleStatuses) if (filters.has(s)) n += 1;
    return n;
  }, [filters, vehicleStatuses]);
